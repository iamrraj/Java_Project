...
*/
   public void getEmpTable() {
        DefaultTableModel emp = new DefaultTableModel(new String[]{"ID", "Name","Time","Language","Department","Semester"}, 0);
        try {
            st = conn.createStatement();
            ...
            while (rs.next()) {
               ...
               String col3 = rs.getString("hour")+" : "+rs.getString("min");
               String col4 = rs.getString("language");
               String col5 = rs.getString("department");
               String col6 = rs.getString("semester");
    ...
    private void searchEmp() {
        DefaultTableModel emp = new DefaultTableModel(new String[]{"ID", "Name","Time","Language"}, 0);
        try {
            st = conn.createStatement();
            String sql = null;
            if (jComboBoxSearch.getSelectedItem().equals("Search By : Name")) {
                sql = "SELECT * FROM course WHERE  name like '" + jTextFieldSearch.getText() + "%'";
            }else if (jComboBoxSearch.getSelectedItem().equals("Department")) {
                sql = "SELECT * FROM course WHERE  department like '" + jTextFieldSearch.getText() + "%'";
            }else if (jComboBoxSearch.getSelectedItem().equals("Semester")) {
                sql = "SELECT * FROM course WHERE  semester like '" + jTextFieldSearch.getText() + "%'";
            }
            else {
                sql = "SELECT * FROM course WHERE language like '" + jTextFieldSearch.getText() + "%'";
            }
            rs = st.executeQuery(sql);
            while (rs.next()) {
                String  col1 = rs.getString("id");
    ...
    private void deleteEmp() {
        if (jTableEmp.getSelectedRow() == -1) {
            if (jTableEmp.getSelectedRow() == 0) {
                ...
                Jmessage.setText("You need to select an Course");
            }
        } else {
            int del = JOptionPane.showConfirmDialog(null, "Do you really want to delete this Course?", "DELETE", JOptionPane.YES_NO_OPTION);
            if (del == 0) {
                int selectedRowIndex = jTableEmp.getSelectedRow();
                theQuery("delete from course where id='" + jTableEmp.getModel().getValueAt(selectedRowIndex, 0) + "'");
