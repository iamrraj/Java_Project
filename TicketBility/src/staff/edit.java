...
public class edit extends javax.swing.JFrame {
    ...
    public void getConnection(){
     ...
     public void getEmpTable(){
        DefaultTableModel emp = new DefaultTableModel(new String[]{"ID","Name", "Phone", "Salary","Sex","Email"}, 0);
        try{
            ...
            while(rs.next())
            {
                ...
                String col6 = rs.getString("gender");
                String col7 = rs.getString("email");           
                emp.addRow(new Object[]{col1, col2, col3,  col5, col6,col7});
            }
     ...
     private void searchEmp(){
        ...
        try{
            ...
            if(jComboBoxSearch.getSelectedItem().equals("Search By : Name")){
                sql = "SELECT * FROM staff WHERE name like '"+jTextFieldSearch.getText()+"%' or name like '"+jTextFieldSearch.getText()+"%'";
            }
            else if(jComboBoxSearch.getSelectedItem().equals("ID")){
                sql = "SELECT * FROM staff WHERE ID like '"+jTextFieldSearch.getText()+"%'";
            }
            else if(jComboBoxSearch.getSelectedItem().equals("Department")){
                sql = "SELECT * FROM staff WHERE gender like '"+jTextFieldSearch.getText()+"%'";
            }
            else{
                sql = "SELECT * FROM staff WHERE salary like '"+jTextFieldSearch.getText()+"%'";
            }
            rs = st.executeQuery(sql);
            while(rs.next())
            {
     ...
     private void deleteEmp(){
        if(jTableEmp.getSelectedRow()==-1){
            if (jTableEmp.getSelectedRow()==0){
                ...
                jLabelMessage.setText("You need to select an Staff");
            }
        }
        else{
            int del = JOptionPane.showConfirmDialog(null, "Do you really want to delete this Staff?", "DELETE", JOptionPane.YES_NO_OPTION);
            if(del==0){
                int selectedRowIndex = jTableEmp.getSelectedRow();
               // theQuery();
                theQuery("delete from staff where ID='"+jTableEmp.getModel().getValueAt(selectedRowIndex, 0)+"'");
